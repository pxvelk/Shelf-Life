...
export default (props) => {
    ...
    const createProduct = () => {
        let newItem = {
          ...
          id: "",
          dateAdded: null,
          image: props.item.image_front_small_url,
          amount: amount,
          ingredients: props.item.ingredients_hierarchy,
          allergens: props.item.allergens_hierarchy
        }
        return newItem
    }
    ...
    const submit = () => {
        ...
        if ((newItem.name) && (newItem.code) && (newItem.amount)) {
            setShowConfirm(true)
            setTimeout(() => {
                closeModal()
            }, 300)
            ...
            <View style={styles.nameContainer}>
                    <View style ={styles.imageContainer}>
                        <View style = {styles.imageSmallerContainer}>
                            {(props.item.image_front_small_url) ? (
                                <Image style={{width: "80%", height: "100%", resizeMode:'cover', borderRadius: 5}} source={{uri: props.item.image_front_small_url}}/>
                            )
                            : (
                    ...
                    <View style = {styles.textAndButtons}>
                        <View style ={styles.textContainer}>
                            <Text style = {styles.nameText} numberOfLines = {2}>{props.item.product_name}</Text>
                        </View>
                    </View>
            </View>
